...
var ZeptoAutocomplete = {
    init: function (limit, data, caseSensitive, showFunc, hideFunc, noresultHtml) {
        this.limit = limit;
        this.data = data;
        this.remoteTimeout = 3000;
        this.resultContainer = $('.autocomplete-result');
        this.noresultHtml = noresultHtml;
        this.caseSensitive = typeof caseSensitive !== 'undefined' ? caseSensitive : true;
        this.showFunction = typeof showFunc == 'function' ? showFunc : function(){ this.resultContainer.show(); };
        this.hideFunction = typeof hideFunc == 'function' ? hideFunc : function(){ this.resultContainer.hide(); };
    ...
    autocomplete: function (options) {
        ...
        if (this._isLocal(options)) {
            this._initLocal(options.limit, options.data, options.caseSensitive, options.show, options.hide, options.noresultHtml);
            return;
        }
        if (this._isRemote(options)) {
            this._initRemote(options.limit, options.data, options.caseSensitive, options.show, options.hide, options.noresultHtml);
        }
    },
    ...
    _initLocal: function (limit, data, caseSensitive, showFunc, hideFunc, noresultHtml) {
    ...
    _successHandler: function (data) {
        if (data === undefined || data.length <= 0){
            this._clearResults();
            this.resultContainer.html(this.noresultHtml);
            this.showFunction();
            return;
        }
